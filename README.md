...
| `create-agent` | — | Create a new Fyso agent with a guided prompt wizard |
...
- **Claude Code:** `/fyso:create-agent` drafts an agent role/prompt with a guided wizard, and `/fyso:create-team` assembles agents into a team.
- **OpenCode:** ask the agent to use `fyso-create-agent` first, then `fyso-create-team` to assemble teams.
