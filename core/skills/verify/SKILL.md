...
```
the verification workflow phase 1          # Verify phase 1
the verification workflow phase 2 --strict # Strict mode: fail on any warning
```
...
- Suggest re-running the build workflow for the affected plan
...
Next steps:
  - Fix gaps: the build workflow phase N plan M
  - Move on: the phase planning workflow N+1
```
...
4. **Don't fix things during verification.** Only report. Fixing is the build workflow's job.
