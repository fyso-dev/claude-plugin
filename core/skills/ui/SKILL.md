...
```
BACKEND FIRST (via the plan workflow + the build workflow)
  └─▶ Phase 1..N: entities, rules, channels — all published in tenant

UI DESIGN (this skill — design only, no code yet)
  ├─▶ the UI planning workflow      → discovery questions → UI-SPEC.md
  │     or
  │   the UI infer workflow     → AI infers everything → UI-SPEC.md + MOCKUPS + CONTRACTS
  ├─▶ the UI mockup workflow    → ASCII wireframes → UI-MOCKUPS.md (approved)
  └─▶ the UI contracts workflow → API contracts, roles, auth → UI-CONTRACTS.md

UI BUILD (only when backend is fully built)
  └─▶ the UI build workflow     → generate frontend code

UI AUDIT (after build)
  └─▶ the UI audit workflow     → security, domain, permissions, UX checks
```

**Gate rule:** the UI build workflow is BLOCKED unless:
...
```
the UI workflow plan              # Discovery + design: ask questions, generate UI spec
the UI workflow infer "<desc>"    # AI-inferred: generate everything from short description
the UI workflow mockup            # Generate ASCII mockups for validation
the UI workflow contracts         # Document API contracts, roles, auth flows
the UI workflow build             # Generate the actual UI code (blocked until backend ready)
the UI workflow audit             # Audit: security, domain, permissions, UX problems
the UI workflow all               # Full pipeline: plan → mockup → contracts → build
```
...
```
the UI workflow infer "Panel admin para consultorio de fonoaudiologia"
the UI workflow infer "Portal de clientes para estudio contable"
the UI workflow infer "Catalogo publico de productos con area de admin"
```
...
Próximo paso:
  the UI build workflow — generar el frontend
```
...
Construí el backend primero:
  the build workflow phase 2   → sesiones + facturas

Cuando termines, volvé con:
  the UI build workflow
```

Never suggest the UI build workflow if the backend is not ready.
...
**1. Design docs exist:**
- `.planning/UI-SPEC.md` → if missing: `run the UI planning workflow or the UI infer workflow first`
- `.planning/UI-MOCKUPS.md` → if missing: `run the UI mockup workflow first`
- `.planning/UI-CONTRACTS.md` → if missing: `run the UI contracts workflow first`
...
- After the UI workflow build` to validate the generated code
- After the UI workflow infer` to verify the AI's decisions
...
```
the UI workflow audit              # Full audit (all dimensions)
the UI workflow audit security     # Security only
the UI workflow audit domain       # Domain logic only
the UI workflow audit permissions  # Role/permission issues only
the UI workflow audit ux           # UX/accessibility issues only
```
...
- For deep dives:
  ...
  - `referencethe UI workflow-components.md` — @fyso/ui component API reference (DataGrid, DynamicForm, RecordDetail)
...
Next steps:
  - the deploy workflow to publish to sites.fyso.dev
