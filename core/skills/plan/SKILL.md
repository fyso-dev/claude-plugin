...
Action: Use the UI infer workflow with the app description to design and generate the frontend.
...
Next steps:
  ...
  - Execute: use the build workflow for phase N
  - Or discuss more: use the phase discussion workflow for phase N
```

If no `.planning/UI-SPEC.md` exists, add:
```
⚠️  UI no definida todavía.
Cuando termines de construir las entidades, usa el UI infer workflow con la descripción de tu app.
