...
You create **channels** that expose your Fyso app's data as discoverable tools. Other agents (the host agent, bots, apps) can find your channel and execute operations against your entities — without writing code.
...
```
the expose workflow                  # Interactive: ask what to expose
the expose workflow pacientes        # Expose CRUD tools for pacientes entity
the expose workflow all              # Expose tools for all published entities
```
